...
class TestingStreamsSpec extends TestKit(ActorSystem("TestingAkkaStreams"))
  ...
  "A simple stream"  should {
    ...
    "integrate with a test-actor-based sink" in {
      ...
      streamUnderTest.to(Sink.foreach(n => probe.testActor ! n)).run()
    ...
    "integrate with Streams TestKit Source" in {
      ...
      val sinkUnderTest = Sink.foreach[Int] {
        case 45 => throw new RuntimeException("bad party")
        case _ =>
      }
      ...
      resultFuture.onComplete {
        case Success(_) => fail("The sink under test should have thrown an exception on 45")
    ...
    "test flows with a test source and a test sink" in  {
      ...
      subscriber
        .request(4) // don't forget to do this, you need to say how many requests for the subscriber
